...
MC_TARGET_FUNC void mc_blas_srotm(int n, float * x, int incx, float * y, int incy, const float param[5])
{
	...
	if (incx == incy && incx > 0) {
		nsteps = n * incx;
		if (flag < zero) {
			h11 = MC_BLAS_VAT(param, 2);
			h12 = MC_BLAS_VAT(param, 4);
			h21 = MC_BLAS_VAT(param, 3);
			h22 = MC_BLAS_VAT(param, 5);
			for (i = 1; incx < 0 ? i >= nsteps : i <= nsteps; i += incx) {
				w                 = MC_BLAS_VAT(x, i);
				z                 = MC_BLAS_VAT(y, i);
				MC_BLAS_VAT(x, i) = w * h11 + z * h12;
				MC_BLAS_VAT(y, i) = w * h21 + z * h22;
			}
		} else if (flag == zero) {
			h12 = MC_BLAS_VAT(param, 4);
			h21 = MC_BLAS_VAT(param, 3);
			for (i = 1; incx < 0 ? i >= nsteps : i <= nsteps; i += incx) {
				w                 = MC_BLAS_VAT(x, i);
				z                 = MC_BLAS_VAT(y, i);
				MC_BLAS_VAT(x, i) = w + z * h12;
				MC_BLAS_VAT(y, i) = w * h21 + z;
			}
		} else {
			h11 = MC_BLAS_VAT(param, 2);
			h22 = MC_BLAS_VAT(param, 5);
			for (i = 1; incx < 0 ? i >= nsteps : i <= nsteps; i += incx) {
				w                 = MC_BLAS_VAT(x, i);
				z                 = MC_BLAS_VAT(y, i);
				MC_BLAS_VAT(x, i) =  w * h11 + z;
				MC_BLAS_VAT(y, i) = -w + h22 * z;
			}
		}
	} else {
		kx = 1;
		...
		if (flag < zero) {
			...
			for (i = 1; i <= n; ++i) {
				w                  = MC_BLAS_VAT(x, kx);
				z                  = MC_BLAS_VAT(y, ky);
				MC_BLAS_VAT(x, kx) = w * h11 + z * h12;
				MC_BLAS_VAT(y, ky) = w * h21 + z * h22;
				kx                 = kx + incx;
				ky                 = ky + incy;
			}
		} else if (flag == zero) {
			...
			for (i = 1; i <= n; ++i) {
				w                  = MC_BLAS_VAT(x, kx);
				z                  = MC_BLAS_VAT(y, ky);
				MC_BLAS_VAT(x, kx) = w + z * h12;
				MC_BLAS_VAT(y, ky) = w * h21 + z;
				kx                 = kx + incx;
				ky                 = ky + incy;
			}
		} else {
			...
			for (i = 1; i <= n; ++i) {
				w                  = MC_BLAS_VAT(x, kx);
				z                  = MC_BLAS_VAT(y, ky);
				MC_BLAS_VAT(x, kx) =  w * h11 + z;
				MC_BLAS_VAT(y, ky) = -w + h22 * z;
				kx                 = kx + incx;
				ky                 = ky + incy;
			}
		}
