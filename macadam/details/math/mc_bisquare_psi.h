...
MC_TARGET_FUNC float mc_bisquare_psicf(float r, float c)
{
	if (mc_fabsf(r) <= c) {
...
MC_TARGET_FUNC double mc_bisquare_psic(double r, double c)
{
	if (mc_fabs(r) <= c) {
...
MC_TARGET_FUNC long double mc_bisquare_psicl(long double r, long double c)
{
	if (mc_fabsl(r) <= c) {
...
MC_TARGET_FUNC float mc_bisquare_psif(float r)
{
	const float c = 4.685f;
	return mc_bisquare_psicf(r, c);
}

MC_TARGET_FUNC double mc_bisquare_psi(double r)
{
	const double c = 4.685;
	return mc_bisquare_psic(r, c);
}

MC_TARGET_FUNC long double mc_bisquare_psil(long double r)
{
