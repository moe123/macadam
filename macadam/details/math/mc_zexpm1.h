...
MC_TARGET_PROC void mc_zexpm1f(float * c_r, float * c_i
	...
	if ((mc_fabsf(a_r) >= 1.0f) || (mc_fabsf(a_i) >= 1.0f)) {
		...
		const float zm1x = mc_expm1f(a_r);
		const float zp1x = 1.0f + zm1x;
		*c_r             = zm1x - 2.0f * zp1x * mc_raise2f(mc_sinf(0.5f * a_i));
		*c_i             = zp1x * mc_sinf(a_i);
	}
}
...
MC_TARGET_PROC void mc_zexpm1(double * c_r, double * c_i
	...
	if ((mc_fabs(a_r) >= 1.0) || (mc_fabs(a_i) >= 1.0)) {
		...
		const double zm1x = mc_expm1(a_r);
		const double zp1x = 1.0 + zm1x;
		*c_r              = zm1x - 2.0 * zp1x * mc_raise2(mc_sin(0.5 * a_i));
		*c_i              = zp1x * mc_sin(a_i);
	}
}
...
MC_TARGET_PROC void mc_zexpm1l(long double * c_r, long double * c_i
	...
	if ((mc_fabsl(a_r) >= 1.0L) || (mc_fabsl(a_i) >= 1.0L)) {
		...
		const long double zm1x = mc_expm1l(a_r);
		const long double zp1x = 1.0L + zm1x;
		*c_r                   = zm1x - 2.0L * zp1x * mc_raise2l(mc_sinl(0.5L * a_i));
		*c_i                   = zp1x * mc_sinl(a_i);
	}
}
