...
#	if MC_TARGET_CPP98
	...
	return (
		::logf(x) - 0.5f / x
		+ MCK_KF(MCK_ZETAM1)  * p
		+ MCK_KF(MCK_ZETAM3)  * ::powf(p, 2)
		+ MCK_KF(MCK_ZETAM5)  * ::powf(p, 3)
		+ MCK_KF(MCK_ZETAM7)  * ::powf(p, 4)
		+ MCK_KF(MCK_ZETAM9)  * ::powf(p, 5)
		+ MCK_KF(MCK_ZETAM11) * ::powf(p, 6)
	);
#	else
	const float p = powf(x, -2.0f);
	return (
...
#	if MC_TARGET_CPP98
	...
	return (
		::log(x) - 0.5 / x
		+ MCK_K(MCK_ZETAM1)  * p
		+ MCK_K(MCK_ZETAM3)  * ::pow(p, 2)
		+ MCK_K(MCK_ZETAM5)  * ::pow(p, 3)
		+ MCK_K(MCK_ZETAM7)  * ::pow(p, 4)
		+ MCK_K(MCK_ZETAM9)  * ::pow(p, 5)
		+ MCK_K(MCK_ZETAM11) * ::pow(p, 6)
	);
#	else
	const double p = pow(x, -2.0);
	return (
...
#	if MC_TARGET_CPP98
	...
	return (
		::logl(x) - 0.5L / x
		+ MCK_KL(MCK_ZETAM1)  * p
		+ MCK_KL(MCK_ZETAM3)  * ::powl(p, 2)
		+ MCK_KL(MCK_ZETAM5)  * ::powl(p, 3)
		+ MCK_KL(MCK_ZETAM7)  * ::powl(p, 4)
		+ MCK_KL(MCK_ZETAM9)  * ::powl(p, 5)
		+ MCK_KL(MCK_ZETAM11) * ::powl(p, 6)
	);
#	else
	const long double p = powl(x, -2.0L);
	return (
