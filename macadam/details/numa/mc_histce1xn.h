...
MC_TARGET_FUNC float mc_histce1xnf(int npts, int binw, int b, const int * h)
{
	...
	if (c > 0 && npts > 1) {
		for (;i < npts; i++) {
			const float p  = mc_cast_expr(float, h[i]) / c;
			e             -= p > 0.0f ? p * (b > 0 ? mc_log2f(p) : mc_logbasef(p, b)) : 0.0f;
		}
	}
	return e;
}
...
MC_TARGET_FUNC double mc_histce1xn(int npts, int binw, int b, const int * h)
{
	...
	if (c > 0 && npts > 1) {
		for (;i < npts; i++) {
			const double p  = mc_cast_expr(double, h[i]) / c;
			e              -= p > 0.0 ? p * (b > 0 ? mc_log2(p) : mc_logbase(p, b)) : 0.0;
		}
	}
	return e;
}
...
MC_TARGET_FUNC long double mc_histce1xnl(int npts, int binw, int b, const int * h)
{
	...
	if (c > 0 && npts > 1) {
		for (;i < npts; i++) {
			const long double p  = mc_cast_expr(long double, h[i]) / c;
			e                   -= p > 0.0L ? p * (b > 0 ? mc_log2l(p) : mc_logbasel(p, b)) : 0.0L;
		}
	}
	return e;
}
