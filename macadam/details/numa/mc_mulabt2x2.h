...
#ifndef MC_MULABT2X2_H
#define MC_MULABT2X2_H

#pragma mark - mc_mulabt2x2 -
...
//!# c=a*b'
	...
	c[2] = (mc_cast(double, a[2]) * mc_cast(double, b[0])) + (mc_cast(double, a[3]) * mc_cast(double, b[1]));
	c[3] = (mc_cast(double, a[2]) * mc_cast(double, b[2])) + (mc_cast(double, a[3]) * mc_cast(double, b[3]));
}

MC_TARGET_FUNC void mc_mulabt2x2fd(double * MC_TARGET_RESTRICT c, const float a[4], const double b[4])
{
//!# c=a*b'
	c[0] = (mc_cast(double, a[0]) * b[0]) + (mc_cast(double, a[1]) * b[1]);
	c[1] = (mc_cast(double, a[0]) * b[2]) + (mc_cast(double, a[1]) * b[3]);

	c[2] = (mc_cast(double, a[2]) * b[0]) + (mc_cast(double, a[3]) * b[1]);
	c[3] = (mc_cast(double, a[2]) * b[2]) + (mc_cast(double, a[3]) * b[3]);
}

MC_TARGET_FUNC void mc_mulabt2x2df(double * MC_TARGET_RESTRICT c, const double a[4], const float b[4])
{
//!# c=a*b'
	c[0] = (a[0] * mc_cast(double, b[0])) + (a[1] * mc_cast(double, b[1]));
	c[1] = (a[0] * mc_cast(double, b[2])) + (a[1] * mc_cast(double, b[3]));

	c[2] = (a[2] * mc_cast(double, b[0])) + (a[3] * mc_cast(double, b[1]));
	c[3] = (a[2] * mc_cast(double, b[2])) + (a[3] * mc_cast(double, b[3]));
}
